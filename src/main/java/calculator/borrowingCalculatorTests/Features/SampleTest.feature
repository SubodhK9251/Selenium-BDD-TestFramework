Feature: Borrowing calculator functionality test
  ...
  Scenario Outline: Verify calculator provides borrowing estimate for a person with valid set of data
    ...
    Then Verify borrowing calculator provides an estimate that user can borrow and validate the estimate "<estimate>" is displayed
    ...
    Examples: 
      ...
      |  Single        |    0               |     Home to live in |    80000     |    10000         |     500             |      0                  |     100                  |          0           |     10000           | $428,000|
      |  Single        |    0               |     Home to live in |    70000     |    8000          |     700             |      0                  |     100                  |          0           |     10000           | $347,000|
  ...
  Scenario Outline: Verify clicking on start over button clears the form
    ...
    Examples:
      | annualIncome | otherAnnualIncome| monthlyLivingExpense| homeLoanMonthlyRepayment| otherLoanMonthlyRepayment|otherMonthlyCommitment| totalCreditCardLimit|
